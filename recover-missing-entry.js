  ...
  if (mode === "1") {
    ...
    if (incomplete.length === 0) {
      ...
      incomplete.forEach((entry, idx) => {
        console.log(
          `${idx + 1}. User: ${entry.username}, Text: "${entry.text}", Seed: ${entry.seed}`,
        );
      });
      ...
      if (fixId) {
        ...
        if (entry) {
          const newRarity = await question(
            `Enter rarity for "${entry.text}" (COMMON, RARE, EPIC, LEGENDARY): `,
          );
          if (
