...
async function main() {
  ...
  if (missingRarity.length === 0) {
    ...
    missingRarity.forEach((entry) => {
      console.log(
        `  ⚠️  ${entry.username} | "${entry.text}" | Seed: ${entry.seed} | ${new Date(entry.createdAt).toLocaleString()}`,
      );
    });
    console.log();
  }
  ...
  if (missingImages.length === 0) {
    ...
    missingImages.forEach((entry) => {
      console.log(
        `  ⚠️  ${entry.username} | "${entry.text}" | Seed: ${entry.seed}`,
      );
    });
    console.log();
  }
  ...
  console.log(
    `Orphaned Entries - Not Casted & Missing Images (${orphaned.length}):`,
  );
  ...
  Array.from(recentByDay.entries()).forEach(([day, entries]) => {
    ...
    entries.slice(0, 3).forEach((e) => {
      console.log(
        `  • ${e.username} | "${e.text}" | ${e.rarity || "NO RARITY"}`,
      );
    });
  });
  console.log();
  ...
  allEntries.forEach((entry) => {
    ...
    const lastDate = stat.lastCreated
      ? new Date(stat.lastCreated).getTime()
      : 0;
  ...
  topUsers.forEach(([user, stat]) => {
    console.log(
      `  ${user.padEnd(20)} | ${stat.count} logos | Best: ${stat.rarest || "N/A"}`,
    );
  });
  console.log();
  ...
  const issues =
    missingRarity.length + missingImages.length + duplicates.length;
  ...
  console.log(
    `Health Score: ${healthScore}/100 ${healthScore >= 90 ? "✅" : healthScore >= 80 ? "⚠️" : "❌"}`,
  );
  console.log();
  ...
  if (issues === 0) {
    ...
    if (missingImages.length > 0) {
      console.log(
        `  2. Investigate ${missingImages.length} entry(ies) missing image URLs`,
      );
    }
    if (duplicates.length > 0) {
      console.log(
        `  3. Review and remove ${duplicates.length} duplicate entry(ies)`,
      );
    }
  }
  console.log();
