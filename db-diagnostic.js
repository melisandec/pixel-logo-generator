...
async function main() {
  ...
  if (oldest && newest) {
    ...
    const daysDiff = Math.floor((newest.createdAt - oldest.createdAt) / (1000 * 60 * 60 * 24));
  ...
  rarityOrder.forEach((rarity) => {
    ...
    if (count > 0) {
      ...
      console.log(`${rarity.padEnd(12)}: ${String(count).padStart(3)} (${pct}%) ${bar}`);
    }
  });
  console.log();
  ...
  console.log(`Entries not casted: ${genCount - (await prisma.generatedLogo.count({ where: { castUrl: { not: null } } }))}`);
